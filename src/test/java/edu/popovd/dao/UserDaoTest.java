...
class UserDaoTest {
    ...
    void isItPossible() {
        ...
        List<String> names = results.stream().map(t -> t.get(0, User.class).fullName()).collect(toList());
        assertThat(names).contains("Sergey Brin", "Steve Jobs");

        List<Double> averagePayments = results.stream().map(t -> t.get(1, Double.class)).collect(toList());
